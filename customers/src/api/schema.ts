...
export const getAllSchemasCall = async (
  tenant: string,
  ...
  const url = queryString
    ? `schema/${tenant}/schemas?${queryString}`
    : `schema/${tenant}/schemas`
  const { data } = await api.get<Schema[]>(url)
  return data
}
...
export const useSchemaApi = () => {
  ...
  const getUnassignedSchemas = useCallback(async (): Promise<Schema[]> => {
    if (!tenant) {
      return Promise.reject('No tenant provided')
    }
    const allSchemas = await getAllSchemasCall(tenant, {
      pageNumber: 1,
      pageSize: 500,
    })
