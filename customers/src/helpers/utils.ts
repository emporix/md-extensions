...
/** @see Label chip option shape used in configuration UI */
...
export const deepMerge = <T = unknown>(a: T, b: T): T => {
  if (b === undefined) return a
  if (a === null || b === null) return b as T
...
export const deepFieldsRemove = <T>(
  value: T,
  keysToRemove: ReadonlySet<string>,
  opts: { removeEmpty?: boolean } = {}
): unknown => {
  const { removeEmpty = false } = opts
  if (Array.isArray(value)) {
    const arr = value.map((v) => deepFieldsRemove(v, keysToRemove, opts))
    return removeEmpty ? arr.filter((v) => v !== undefined) : arr
  }
  if (value && typeof value === 'object') {
    const out: Record<string, unknown> = {}
    for (const [k, v] of Object.entries(value as Record<string, unknown>)) {
      if (keysToRemove.has(k)) continue
      const next = deepFieldsRemove(v, keysToRemove, opts)
      if (!(removeEmpty && next === undefined)) out[k] = next
    }
    if (removeEmpty && Object.keys(out).length === 0) return undefined
    return out
  }
  return value
}
