    ...
    switch (type) {
      ...
      case "CUSTOMER_SEGMENT":
        return {
          ...
          label: (item: unknown) =>
            getContentLangValue((item as Segment).name) || (item as Segment).id,
          permissions: {
            canView: !!permissions?.segments?.viewer,
            canManage: !!permissions?.segments?.manager,
          },
          navigationPath: (id: string) => `/customer-management/segments/${id}`,
      ...
      case "CATEGORY":
        return {
          ...
          label: (item: unknown) => {
            ...
            return getContentLangValue(c.localizedName) || c.code || c.id;
          },
          permissions: {
