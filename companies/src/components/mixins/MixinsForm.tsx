...
import MixinSchemaInfoButton from "./MixinSchemaInfoButton";
...
const MixinsForm = (props: MixinFormProps) => {
  ...
  const showInlineActions = !bulkRegistry || bulkRegistrationId === undefined;
  ...
  return (
    <>
      <SectionBox
        name={name}
        titleAccessory={
          <MixinSchemaInfoButton
            schemaUrl={metadata.url}
            schemaVersion={metadata.version}
            newerSchemaUrl={newerMetadata?.url}
            newerSchemaVersion={newerMetadata?.version}
          />
        }
        actions={
          showInlineActions ? (
