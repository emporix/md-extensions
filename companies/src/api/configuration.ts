...
export const updateSingleConfiguration = async (
  ...
  try {
    ...
    return await createSingleConfiguration(
      tenant,
      configurationKey,
      data.value,
    );
...
export const deleteSingleConfiguration = async (
  tenant: string,
  configurationKey: string,
): Promise<unknown> => {
      ...
      if (tenant) {
        try {
          ...
          if (error instanceof AxiosError && error.response?.status === 404) {
            return await createSingleConfiguration(tenant, key, payload.value);
          }
          throw error;
        }
      } else {
