...
const findCorsOrigins = (configSource) => {
  ...
  return {
    kind: 'inline',
    origins: [
      ...new Set(inlineArrays.flatMap((entry) => parseOriginList(entry[1]))),
    ],
...
const addOriginToConfig = (configSource, origin, corsInfo) => {
  ...
  for (const match of [...corsInfo.matches].reverse()) {
    ...
    updated =
      ...
      replacement +
      updated.slice(match.index + match[0].length)
  }
  return updated
}
...
const main = async () => {
  ...
  console.error(
    `CORS check failed: "${origin}" is not whitelisted (mode: ${mode}).`
  )
  ...
  console.info(
    `Added "${origin}" to CORS whitelist in vite.config.ts. Continuing build.`
  )
}
