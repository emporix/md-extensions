...
export const isArrayMixinField = (obj: unknown): obj is ArrayMixinField => {
  ...
  return (
    ...
    (typeof candidate.value === 'string' ||
      ...
      (Array.isArray(candidate.value) &&
        candidate.value.every((val: unknown) => isLocalizedMixin(val))))
  )
}

export const isLocalizedMixin = (obj: unknown): obj is LocalizedMixin => {
  if (typeof obj !== 'object' || obj === null) {
    return false
  }
  const candidate = obj as Record<string, unknown>
...
export const convertToIdValuePair = (obj: any): any => {
  // Mixin payloads are schema-driven and arbitrarily shaped; `any` is the
  // pragmatic type here, as in the source implementation.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const newObj: any = {}
...
export const mapLocalizedMixinToLocalized = (
  ...
  // @ts-expect-error reduce over a heterogeneous mixin array
