...
export const McpServerForm: React.FC<McpServerFormProps> = React.memo(
  ({ onAdd, onCancel, editingServer }) => {
    ...
    const [newEmporixMcp, setNewEmporixMcp] = useState<McpKey>(
      editingServer?.type === 'predefined' && editingServer.domain
        ? editingServer.domain
        : 'order'
    )
    ...
    const [newCustomName, setNewCustomName] = useState(
      editingServer?.type === 'custom' && editingServer.name
        ? editingServer.name
        : ''
    )
    const [newCustomUrl, setNewCustomUrl] = useState(editingServer?.url || '')
          ...
          <>
            ...
            <div className="form-field">
              ...
              <MultiSelect
                ...
                options={(
                  MCP_SERVERS[newEmporixMcp]?.tools?.map((tool: string) => ({
                    label: tool,
                    value: tool,
                  })) || []
                ).sort((a, b) => a.label.localeCompare(b.label))}
