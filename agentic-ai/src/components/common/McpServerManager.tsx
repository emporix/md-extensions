...
export const McpServerManager: React.FC<McpServerManagerProps> = memo(({
  ...
  const handleAddMcp = useCallback((server: McpServer) => {
    onMcpServersChange([...mcpServers, server]);
    setShowAddMcp(false);
  }, [mcpServers, onMcpServersChange]);

  const handleUpdateMcp = useCallback((index: number, server: McpServer) => {
  ...
  const handleDeleteMcp = useCallback((index: number) => {
  ...
  const handleEditMcp = useCallback((index: number, _server: McpServer) => {
  ...
  return (
    <div className="mcp-servers-section">
      <div className="mcp-servers-header">
        <span className="mcp-servers-title">{t('mcp_servers', 'MCP Servers')}</span>
        <button 
