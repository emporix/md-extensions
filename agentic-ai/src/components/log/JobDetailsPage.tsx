...
const JobDetailsPage: React.FC<JobDetailsPageProps> = ({ appState }) => {
  ...
  const [lastFetchedRequestId, setLastFetchedRequestId] = useState<string | null>(null);
  ...
  const fetchLogsByRequestId = useCallback(async (requestId: string) => {
    try {
  ...
  useEffect(() => {
    if (selectedJob && selectedJob.requestId && selectedJob.requestId !== lastFetchedRequestId) {
