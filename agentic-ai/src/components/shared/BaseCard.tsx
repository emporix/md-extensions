...
const BaseCard: React.FC<BaseCardProps> = ({
        ...
        <div className="card-header-right" onClick={(e) => e.stopPropagation()}>
          {showStatusDot ? (
            <div className="status-dot-indicator">
              <span
                className={`status-dot ${isActive ? 'status-dot-active' : 'status-dot-inactive'}`}
              ></span>
  ...
  const cardFooter = (
    <div className="custom-agent-card-footer">
      {children && <div className="card-children-wrapper">{children}</div>}
      ...
      <div className="card-actions-row">
        {actions.map((action, index) => (
          ...
          >
            ...
            {action.className !== 'remove-button' &&
              action.className !== 'export-button' &&
              action.label}
          </button>
        ))}
      </div>
    ...
    <div title={id ? `ID: ${id}` : undefined}>
      ...
      >
        <div className="agent-content">
          ...
          <div className="agent-tags">{contentBadges || renderBadge()}</div>
        </div>
      </Card>
    </div>
  )
