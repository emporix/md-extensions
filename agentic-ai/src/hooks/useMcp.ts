...
export const useMcp = (appState: AppState) => {
  ...
  const [pendingToggle, setPendingToggle] = useState<{ mcpServerId: string; enabled: boolean } | null>(null);
  ...
  const toggleMcpServerActive = useCallback(async (mcpServerId: string, enabled: boolean) => {
    try {
      const currentMcpServer = mcpServers.find(server => server.id === mcpServerId);
      if (!currentMcpServer) {
      ...
      showSuccess(`MCP Server ${enabled ? 'activated' : 'deactivated'} successfully!`);
      ...
      const errorMessage = formatApiError(err, 'Failed to update MCP server status');
  ...
  }, [mcpServers, mcpService, showSuccess, showError]);
  ...
  }, [pendingToggle, mcpService, showSuccess, showError]);
