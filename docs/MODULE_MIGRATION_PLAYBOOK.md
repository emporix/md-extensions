...
| 2026-08-18 | Mixin form **defaults** must match MD `MixinsForm.prepareData` / `defaultValueForMixinItem` (date/dateTime → `new Date().toISOString()`, enum → first option), not empty strings — the Returns API often omits saved `mixins`, so empty defaults look like a loading bug. Port `defaultValueForMixinItem` + fix `createForm` nested-key assignment | Assuming empty mixin payload means fix API binding; using native date inputs when CL `Calendar` exists | Remotes with mixin form tabs |
